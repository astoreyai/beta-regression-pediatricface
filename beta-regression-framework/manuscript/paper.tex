...
\end{align}
where $\phi > 0$ is a constant precision parameter shared across all subjects and timepoints (variable precision models where $\phi$ depends on covariates are possible but add substantial complexity and require larger samples).
...
Synthetic longitudinal data was generated based on the age-specific patterns reported in the YFA study~\cite{bahmani2023}. For each age group, we first computed age-specific mean trajectories $\mu_{ij}$ incorporating subject-level random effects $u_i \sim N(0, \sigma^2_u)$ with $\sigma_u = 0.03$, then drew TAR values from the beta distribution:
\begin{equation}
\text{TAR}_{ij} \sim \text{Beta}(\mu_{ij} \cdot \phi, (1-\mu_{ij}) \cdot \phi)
\end{equation}
with precision parameter $\phi = 50$. This two-step process ensures properly bounded outcomes with realistic variance structure $\text{Var}(\text{TAR}_{ij}) = \mu_{ij}(1-\mu_{ij})/(1+\phi)$. Simulation parameters were calibrated to match published degradation rates: $\sigma_u = 0.03$ corresponds to between-subject variation observed in Bahmani et al., while $\phi = 50$ reflects the precision observed in longitudinal biometric studies. Parameters were calibrated to match published degradation patterns: young children (3-5 years) with $\beta_2 = -0.35$ for steep decline, middle children (5.5-7 years) with $\beta_2 = -0.08$ for stability, and older children (7.5-9 years) with $\beta_2 = -0.20$ and delayed onset via the interaction term. We simulated 100 subjects per age group (300 total) with observations at enrollment and annually for 8 years, matching the YFA collection structure.
